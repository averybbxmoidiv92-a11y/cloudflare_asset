...
let config_JSON, 缓存SOCKS5白名单 = null,
    调试日志打印 = false;
...
export default {
    async fetch(request, env, ctx) {
        ...
        let 默认反代IP = (`${request.cf.colo}.${特征码字典[0]}.${特征码字典[1]}SsSs.nEt`).toLowerCase(),
            默认反代兜底 = true;
        if (env.PROXYIP) {
        ...
        if (访问路径 === 'version') { // 版本信息接口
            ...
            if (uuidRegex.test(请求UUID)) {
                ...
                if (请求前8总和 === 目标前8总和 && 请求UUID.slice(-12) === 目标UUID.slice(-12)) return new Response(JSON.stringify({
            ...
            if (env.KV && typeof env.KV.get === 'function') {
                ...
                if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') { //快速订阅
                    ...
                    if (authCookie == await MD5MD5(UA + 加密秘钥 + 管理员密码)) return new Response('重定向中...', {
                        status: 302,
                        headers: {
                            'Location': '/admin'
                        }
                    ...
                    if (request.method === 'POST') {
                        ...
                        if (输入密码 === (typeof 管理员密码 === 'string' ? 管理员密码.replace(/[\r\n]/g, '') : 管理员密码)) {
                            ...
                            const 响应 = new Response(JSON.stringify({
                                success: true
                            }), {
                                status: 200,
                                headers: {
                    ...
                    if (!authCookie || authCookie !== await MD5MD5(UA + 加密秘钥 + 管理员密码)) return new Response('重定向中...', {
                        status: 302,
                        headers: {
                            'Location': '/login'
                        }
                    ...
                    if (访问路径 === 'admin/log.json') { // 读取日志内容
                        ...
                        if (!代理协议) return new Response(JSON.stringify({
                            error: '缺少代理参数'
                        }), {
                            status: 400,
                            headers: {
                        ...
                        if (订阅类型 === 'mixed') {
                            ...
                            try {
                                const response = await fetch(订阅转换URL, {
                                    headers: {
                                        'User-Agent': 'Subconverter for ' + 订阅类型 + ' edge' + 'tunnel (https://github.com/' + 特征码字典[1] + '/edge' + 'tunnel)'
                                    }
                                });
                                if (response.ok) {
                    ...
                    const cookies = request.headers.get('Cookie') || '';
                    const authCookie = cookies.split(';').find(c => c.trim().startsWith('auth='))?.split('=')[1];
                    if (authCookie && authCookie == await MD5MD5(UA + 加密秘钥 + 管理员密码)) return fetch(new Request('https://speed.cloudflare.com/locations', {
                        headers: {
        ...
        if (伪装页URL === '1101') return new Response(await html1101(url.host, 访问IP), {
            status: 200,
            headers: {
        ...
        return await nginx(env, request);
    }
};
///////////////////////////////////////////////////////////////////////XHTTP传输数据///////////////////////////////////////////////
...
async function 读取XHTTP首包(reader, token) {
    ...
    while (true) {
        ...
        if (木马结果.状态 === 'ok') return {
            ...木马结果.结果,
            reader
        };

        const 魏烈思结果 = 尝试解析魏烈思首包(当前数据);
        if (魏烈思结果.状态 === 'ok') return {
            ...魏烈思结果.结果,
            reader
        };
    ...
    if (最终木马结果.状态 === 'ok') return {
        ...最终木马结果.结果,
        reader
    };
    const 最终魏烈思结果 = 尝试解析魏烈思首包(最终数据);
    if (最终魏烈思结果.状态 === 'ok') return {
        ...最终魏烈思结果.结果,
        reader
    };
    return null;
...
async function 连接木马反代(首包数据, TCP连接, 木马反代目标) {
    ...
    const socket = TCP连接({
        hostname: stripIPv6Brackets(木马反代目标.hostname),
        port: 木马反代目标.port
    });
    let writer = null;
    try {
...
function 解析木马请求(buffer, passwordPlainText) {
    ...
    switch (atype) {
        ...
        default:
            return {
                hasError: true, message: `invalid addressType is ${atype}`
            };
    }
...
function 解析魏烈思请求(chunk, token) {
    ...
    switch (addressType) {
        ...
        default:
            return {
                hasError: true, message: `Invalid address type: ${addressType}`
            };
    }
    if (!hostname) return {
        hasError: true,
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID, request = null, 反代上下文 = {}, 允许木马反代 = false, 木马反代首包数据 = null) {
    ...
    async function 并发打开候选连接(候选列表) {
        ...
        const attempts = 候选列表.map(候选 => 打开TCP连接(候选.hostname, 候选.port).then(socket => ({
            socket,
            candidate: 候选
        })));
        let winner = null;
        ...
        if (!useBYOB) {
            ...
            while (true) {
                const {
                    ...
                    value
                } = await reader.read(new Uint8Array(readBuffer, 0, BYOB单次读取上限));
                if (done) break;
                if (!value || value.byteLength === 0) continue;
                hasData = true;
...
async function Singbox订阅配置文件热补丁(SingBox_原始订阅内容, config_JSON = {}) {
    ...
    try {
        ...
        const 添加规则集 = (type, code) => {
            ...
            const route = 确保Route(),
                tag = `${type}-${code}`,
                ruleSet = Array.isArray(route.rule_set) ? route.rule_set : 数组化(route.rule_set);
        ...
        const 迁移规则集字段 = rule => {
            ...
            for (const geosite of 数组化(rule.geosite))
                if (typeof geosite === 'string') tags.push(添加规则集('geosite', geosite));
        ...
        if (Array.isArray(config.outbounds)) {
            ...
            if (!outboundTags.has('REJECT') && 引用REJECT({
...
function 识别运营商(request) {
    ...
    const 命中运营商 = 运营商关键词映射.find(({
        pattern
    }) => pattern.test(组织名称))?.code;
...
async function 获取优选订阅生成器数据(优选订阅生成器HOST) {
    let 优选IP = [],
        其他节点LINK = '',
        格式化HOST = 优选订阅生成器HOST.replace(/^sub:\/\//i, 'https://').split('#')[0].split('?')[0];
    ...
    try {
        const response = await fetch(优选订阅生成器URL, {
            headers: {
                'User-Agent': 'v2rayN/edge' + 'tunnel (https://github.com/' + 特征码字典[1] + '/edge' + 'tunnel)'
            }
        });
...
async function nginx(env, request) {
    try {
        ...
        console.error('Failed to fetch index.html from assets:', e);
    }
    
    // 如果未配置 asset 绑定或读取失败，返回 404 防止报错
    return new Response('404 Not Found', {
        status: 404,
        headers: {
            'Content-Type': 'text/plain; charset=UTF-8'
